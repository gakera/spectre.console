...
And most important of all, the type system guides the developer to do the right thing. Commands can't 
be configured in non-compatible ways, and if a new top-level `add-package` command is to be added
(or moved completely), it's just a single line to change. This makes it easy to 
